...
vstop_mean = 1.05
vstop_delta = 0.03   # range ±0.05
# Generate random vstop values for each cycle (uniform distribution)
vstop_samples = np.random.uniform(low=vstop_mean - vstop_delta,
                                  high=vstop_mean + vstop_delta,
                                  size=num_outputs)
...
for i in range(num_outputs):
        vstop = vstop_samples[i]
        sampled_vstops.append(vstop)
        NoOfVacancies = InitialNoOfColdspots
       ...
       # NoOfHotspots = 3e7
        ...
        fwreset_output_df = pd.DataFrame({
            "X-axis": RampRate * np.array(TimeSeries_FWRESET),
        ...
        bwreset_output_df = pd.DataFrame({
            "X-axis": -AppliedBiasFR - RampRate * np.array(TimeSeries_BWRESET),
...
# Save FW/BW data
fwreset_resistance_df = pd.DataFrame({"FW_Reset_R": fwreset_resistance_values})
bwreset_resistance_df = pd.DataFrame({"Vstop": sampled_vstops, "HRS": bwreset_resistance_values})
...
# 1) CDF of backward reset (HRS)
sorted_bw = np.sort(bwreset_resistance_values)
cdf_bw = np.arange(1, len(sorted_bw) + 1) / len(sorted_bw) * 100

plt.figure(figsize=(6, 4))
plt.plot(sorted_bw, cdf_bw, color="blue", linewidth=2, label="BW Reset HRS CDF")
plt.xscale("log")
plt.xlabel("Resistance (Ohms)")
plt.ylabel("Cumulative Probability (%)")
plt.title("CDF of Backward Reset Resistance")
plt.legend()
...
plt.figure(figsize=(6, 4))
plt.plot(sorted_hotspot_values, cdf_hotspot, color="green", linewidth=2, label="Hotspot CDF")
plt.xscale("log")
...
# 3) Vstop distribution (histogram)
plt.figure(figsize=(6, 4))
plt.hist(sampled_vstops, bins=50, color="orange", alpha=0.7, edgecolor="black")
plt.xlabel("Vstop (V)")
plt.ylabel("Count")
plt.title("Vstop Distribution (Uniform)")

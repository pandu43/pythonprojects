...
EbSet = 3.2#ev
EbReset = 1.7#ev
alphaSet = 1e-9 #prev1e-9
ResetExpo =  4.2
alphaReset1 = 1e20#prev1e14
alphaReset2 = 5e-10#prev2.7e-10
Thickness = 1E-9
...
CurrentSeries_Formation = []
TimeSeries_Formation = []
NoOfHotspotsSeries_Formation = []
NoOfColdspotsSeries_Formation = []
AppliedBiasSeries_Formation = []

CurrentSeries_Annihilation =[]
TimeSeries_Annihilation = []
NoOfHotspotsSeries_Annihilation = []
NoOfColdspotsSeries_Annihilation = []
AppliedBiasSeries_Annihilation = []

R1_series_Formation = []
R2_series_Formation = []
Voltage_series_Formation = []

R1_series_Annihilation = []
R2_series_Annihilation = []
Voltage_series_Annihilation = []
...
def KmcProcess(RampRate, Thickness, NoOfColdspots , NoOfHotspots,
    ...
    while True:
        ...
        if not BiasWasReset:
            ...
            R1_series_Formation.append(R1)
            R2_series_Formation.append(R2)
            Voltage_series_Formation.append(AppliedBias)
        else:
            # Append to RESET (Annihilation) phase
            R1_series_Annihilation.append(R1)
            R2_series_Annihilation.append(R2)
            Voltage_series_Annihilation.append(-AppliedBias)
        ...
        if not BiasWasReset:
           CurrentSeries_Formation.append(CurrentCalculation(AppliedBias, NoOfHotspots))
           TimeSeries_Formation.append(Time)
           NoOfHotspotsSeries_Formation.append(NoOfHotspots)
           NoOfColdspotsSeries_Formation.append(NoOfColdspots)
           AppliedBiasSeries_Formation.append(AppliedBias)
        else:
            CurrentSeries_Annihilation.append(CurrentCalculation(AppliedBias, NoOfHotspots))
            TimeSeries_Annihilation.append(-Time)
            NoOfHotspotsSeries_Annihilation.append(NoOfHotspots)
            NoOfColdspotsSeries_Annihilation.append(NoOfColdspots)
            AppliedBiasSeries_Annihilation.append(AppliedBias)
...
for i in range(num_outputs):
  ...
  #  plt.title('50 cycles ')
    plt.figure(1)
    plt.semilogy(RampRate * np.array(TimeSeries_Formation), CurrentSeries_Formation, '-')
    plt.semilogy(RampRate * np.array(TimeSeries_Annihilation), CurrentSeries_Annihilation, '-')
    plt.xlabel('Voltage (V)')
    plt.ylabel('  Current(A)')
    plt.figure(2)
    plt.semilogy(RampRate * np.array(TimeSeries_Formation), NoOfHotspotsSeries_Formation, '-' ,label='Hotspots')
    plt.semilogy(RampRate * np.array(TimeSeries_Formation), NoOfColdspotsSeries_Formation, '-' ,label='Coldspots')
    plt.semilogy(RampRate * np.array(TimeSeries_Annihilation), NoOfHotspotsSeries_Annihilation, '-', label='Hotspots')
    plt.semilogy(RampRate * np.array(TimeSeries_Annihilation), NoOfColdspotsSeries_Annihilation, '-', label='Coldspots')
    plt.xlabel('Voltage (V)')
    plt.ylabel('  Hotspots')
    plt.legend()
    plt.figure(3)
    plt.semilogy(Voltage_series_Formation, R1_series_Formation, label='R1: Hotspot Formation Rate_SET')
    plt.semilogy(Voltage_series_Formation, R2_series_Formation, label='R2: Hotspot Annihilation Rate_SET')
    plt.semilogy(Voltage_series_Annihilation, R1_series_Annihilation, label='R1: Hotspot Formation Rate_RESET')
    plt.semilogy(Voltage_series_Annihilation, R2_series_Annihilation, label='R2: Hotspot Annihilation Rate_RESET')
    plt.xlabel("Voltage (V)")
    plt.ylabel("Rate (1/s)")
    plt.legend()
    plt.grid(True)

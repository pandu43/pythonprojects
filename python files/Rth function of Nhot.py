        ...
        if (time + tau <= CurrentTimeStep + DeltaT):
           ...
           # print(f"Event occurred at Bias = {AppliedBias:.3f} V, Time = {time:.2e} ")
            ...
            if r < (R1_Set / Rate_Set):
                ...
                upper_bound = max(min(3000, NoOfColdspots), 1)
                ...
                upper_bound = max(min(3000, NoOfHotspots), 1)
                random_numbers = np.random.randint(1, upper_bound + 1)
                #random_numbers=1
...
Rthset_df = pd.DataFrame()
Rthreset_df = pd.DataFrame()
...
for i in range(num_outputs):
    ...
    Rthset_data = {'X-axis': Voltage_series_Set, 'Y-axis':  Rth_series_set}
    Rthset_output_df = pd.DataFrame(Rthset_data)

    Rthreset_data = {'X-axis': Voltage_series_Reset, 'Y-axis': Rth_series_reset}
    Rthreset_output_df = pd.DataFrame(Rthreset_data)
    ...
    Rthset_df = pd.concat([Rthset_df, Rthset_output_df], axis=1)
    Rthreset_df = pd.concat([Rthreset_df, Rthreset_output_df], axis=1)
    ...
    fwset_output_df = pd.DataFrame({'X-axis': fwset_x_axis, 'Y-axis': CurrentSeries_FWSET})
    Bwset_output_df = pd.DataFrame({'X-axis': Bwset_x_axis, 'Y-axis': CurrentSeries_BWSET})
    fwreset_output_df = pd.DataFrame({'X-axis': fwreset_x_axis, 'Y-axis': CurrentSeries_FWRESET})
    bwreset_output_df = pd.DataFrame({'X-axis': bwreset_x_axis, 'Y-axis': CurrentSeries_BWRESET})

    # Calculate resistance and append
    try:
        R_fwset = calculate_Rset(fwset_output_df)
        fwset_resistance_values.append(R_fwset)
    except Exception as e:
        print(f"[{i}] Error calculating fwset resistance:", e)

    try:
        R_Bwset = calculate_Rset(Bwset_output_df)
        Bwset_resistance_values.append(R_Bwset)
    except Exception as e:
        print(f"[{i}] Error calculating Bwset resistance:", e)


    # Safe slicing if enough rows exist
    def safe_slice(df):
        return df.iloc[::2, :] if len(df) >= 2 else df

    fwset_output_df = safe_slice(fwset_output_df)
    Bwset_output_df = safe_slice(Bwset_output_df)
    fwreset_output_df = safe_slice(fwreset_output_df)
    bwreset_output_df = safe_slice(bwreset_output_df)

    # Concatenate to full DataFrames
    fwset_df = pd.concat([fwset_df, fwset_output_df], axis=1)
    Bwset_df = pd.concat([Bwset_df, Bwset_output_df], axis=1)
    fwreset_df = pd.concat([fwreset_df, fwreset_output_df], axis=1)
    bwreset_df = pd.concat([bwreset_df, bwreset_output_df], axis=1)
...
exp_file = "/home/lavanya/Documents/abrupt reset exp.csv"
    #exp_df = pd.read_excel(exp_file, engine='odf')  # Requires odfpy
    #exp_df = pd.read_excel(exp_file)  # Requires odfpy
...
try:
    fwset_df.to_csv(os.path.join(output_folder, f"fwset_data_{timestamp}.csv"), index=False)
    Bwset_df.to_csv(os.path.join(output_folder, f"Bwset_data_{timestamp}.csv"), index=False)
    fwreset_df.to_csv(os.path.join(output_folder, f"fwreset_data_{timestamp}.csv"), index=False)
    bwreset_df.to_csv(os.path.join(output_folder, f"bwreset_data_{timestamp}.csv"), index=False)
    Rthset_df.to_csv(os.path.join(output_folder, f"Rthset_data_{timestamp}.csv"), index=False)
    Rthreset_df.to_csv(os.path.join(output_folder, f"Rthreset_data_{timestamp}.csv"), index=False)
except Exception as e:
    print("Error saving waveform data files:", e)

# Save resistance DataFrames
try:
    ...
    print("Error saving resistance files:", e)


plt.figure(2)
plt.semilogy(Voltage_series_Set, Rth_series_set, '-', color='blue')
plt.semilogy(Voltage_series_Reset, Rth_series_reset, '--', color='blue')

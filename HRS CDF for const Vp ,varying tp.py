...
output_folder = "HRS for const Vp varying tp"
...
pulse_durations = [1.62e-5]  # 5ns → 5ms
pulse_amplitude = 1.77
all_hrs = {}  # dictionary to store results per duration
for pdur in pulse_durations:
    HRS_values = []
    print(f"Running {num_outputs} cycles for pulse_duration = {pdur:.1e} s")
...
plt.xlabel("HRS (Ω)")
plt.ylabel("CDF (%)")
plt.title("CDF of HRS Values for Different Pulse Durations")
plt.legend()

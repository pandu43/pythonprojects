...
def generate_compliance_current(num_outputs):
    ...
    # Generate compliance current within the desired range
     compliance_current_dummy = np.random.normal(mean, std_dev, num_outputs)
     compliance_current = pow(10,compliance_current_dummy)

    # Clip values to ensure they fall within the desired range
    #@ compliance_current = np.clip(compliance_current, lower_bound, upper_bound)
...
def CalculateTheHotspotFormationRate_Set(ElectricField, Eb_Cr, alpha_Cr, Rth_Set, AppliedBias, Current ,kBT_q_Set):
   # kBT_q = calculate_kBT_qSet(AppliedBias, Current, Rs)
    expo = (-(Eb_Cr - alpha_Cr * ElectricField) / kBT_q_Set)
    if expo > 0:
        Rate = nu0
    else:
...
def CalculateTheHotspotAnnihilationRate_Set(ElectricField, AppliedBias, Current, alpha_An1,
        ResetExpo, Rth_Set, alpha_An2, EbReset, kBT_q_Set):

    expo = (-(EbReset - alpha_An1 * pow((AppliedBias * Current),
                                          ResetExpo) - alpha_An2 * ElectricField) / kBT_q_Set)
    if expo > 0:
        Rate = nu01
    else:
...
def CalculateTheHotspotFormationRate_Reset(ElectricField, Eb_Cr, alpha_Cr, Rth_Set, AppliedBias, Current ,kBT_q_Reset):
   # kBT_q = calculate_kBT_qSet(AppliedBias, Current, Rs)
    expo = (-(Eb_Cr - alpha_Cr * ElectricField) / kBT_q_Reset)
    if expo > 0:
        Rate = nu0
    else:
        Rate = nu0 * np.exp(expo)
...
def CalculateTheHotspotAnnihilationRate_Reset(ElectricField, AppliedBias, Current, alpha_An1,
        ResetExpo, Rth_Reset, alpha_An2, EbReset, kBT_q_Reset):

    expo = (-(Eb_An - alpha_An1 * pow((AppliedBias * Current),
                                          ResetExpo) - alpha_An2 * ElectricField) / kBT_q_Reset)
    if expo > 0:
        Rate = nu01
    else:
        Rate = nu01 * np.exp(expo)  # expo is already negative
...
def KmcProcess_FWSet(RampRate, Thickness, NoOfColdspots , NoOfHotspots,
                   Eb_Cr, alpha_Cr, StartBias, Rth_Set, alpha_An1,alpha_An2, ResetExpo, Eb_An , i):
    ...
    while (CurrentCalculation(AppliedBias, NoOfHotspots) <= ComplianceCurrent):
       ...
       # if StepIndex > 1e6:
        ...
        Rate_hotspots_Set = CalculateTheHotspotFormationRate_Set(ElectricField, Eb_Cr, alpha_Cr,Rth_Set,AppliedBias,
                                                         CurrentCalculation(AppliedBias,NoOfHotspots),
                                                       kBT_q_Set )

        Rate_coldspots_Set = CalculateTheHotspotAnnihilationRate_Reset(ElectricField, AppliedBias,
                                                CurrentCalculation(AppliedBias, NoOfHotspots), alpha_An1,
                                                ResetExpo, Rth_Reset, alpha_An2,Eb_An , kBT_q_Set)
...
def KmcProcess_FWReset(RampRate, Thickness, NoOfColdspots , NoOfHotspots,
                   Eb_Cr, alpha_Cr, StartBias, Rth_Set, alpha_An1,alpha_An2, ResetExpo, Eb_An):
    ...
    while (AppliedBias <= 2):
       ...
       # if StepIndex > 1e6:
        ...
        Rate_hotspots_Reset = CalculateTheHotspotFormationRate_Set(ElectricField, Eb_Cr, alpha_Cr, Rth_Set, AppliedBias,
                                                                   CurrentCalculation(AppliedBias, NoOfHotspots),
                                                                   kBT_q_Reset)
        ...
        Rate_coldspots_Reset = CalculateTheHotspotAnnihilationRate_Reset(ElectricField, AppliedBias,
                                                                         CurrentCalculation(AppliedBias, NoOfHotspots),
                                                                         alpha_An1,
                                                                         ResetExpo, Rth_Reset, alpha_An2, Eb_An,
                                                                         kBT_q_Reset)
...
for i in range(num_outputs):
    ...
    ComplianceCurrent = generate_compliance_current(1)[0]
    ...
    timeSet, NoOfColdspotsFS, NoOfHotspotsFS, AppliedBiasFS = KmcProcess_FWSet (RampRate,
                                                                               ...
                                                                               Eb_Cr, alpha_Cr,
                                                                               AppliedBias,Rth_Set,
                                                                               alpha_An1,alpha_An2, ResetExpo, Eb_An ,i)
    ...
    fwset_output_df = fwset_output_df.iloc[::5, :]
    Bwset_output_df = Bwset_output_df.iloc[::5, :]
    fwreset_output_df = fwreset_output_df.iloc[::5, :]
    bwreset_output_df = bwreset_output_df.iloc[::5, :]
    ...
    plt.figure(1)
    plt.semilogy(RampRate * np.array(TimeSeries_FWSET), CurrentSeries_FWSET, '-')
    plt.semilogy(AppliedBiasFS - RampRate * np.array(TimeSeries_BWSET), CurrentSeries_BWSET, '--')
    plt.semilogy(RampRate * np.array(TimeSeries_FWRESET), CurrentSeries_FWRESET, '-')

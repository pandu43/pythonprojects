...
output_folder = "HRS for const tp varying Vp"
...
pulse_duration = 5e-6  # 5ns → 5ms
#pulse_amplitudes = [1.6,1.65,1.7,1.75, 1.8, 1.85,1.9,1.93, 1.95,1.97]
pulse_amplitudes = [1.7,1.75,1.83,1.87,1.9,1.93,1.94,1.95] # values to sweep
all_hrs = {}  # dictionary to store results per duration
for pamp in pulse_amplitudes:
    HRS_values = []
    print(f"Running {num_outputs} cycles for pulse_amplitude = {pamp:.2f} V")
    for i in range(num_outputs):
        ...
        timeReset,NoOfColdspotsFR,NoOfHotspotsFR,AppliedBiasFR = KmcProcess_FWReset (RampRate,
                                                                                ...
                                                                                Eb_Cr,alpha_Cr_Reset,pamp,pulse_duration,
...
for label, values in all_hrs.items():
    ...
    # --- Histogram as PDF in log space ---
    counts, bin_edges = np.histogram(log_data, bins=30, density=True)
    bin_centers = 0.5 * (bin_edges[1:] + bin_edges[:-1])

    # --- Gaussian fit in log space ---
    mu, sigma = norm.fit(log_data)  # fit Gaussian to log10(data)
    pdf_fit = norm.pdf(bin_centers, mu, sigma)

    # --- Plot PDF (log-space) ---

    plt.plot(10 ** bin_centers, pdf_fit, linestyle='-',marker=".", label=label)

plt.xscale('log')  # X-axis in log scale
plt.xlabel("HRS Value (Ω)")
plt.ylabel("Probability Density (PDF)")
plt.title("HRS PDF with Log-Normal Gaussian Fit")
plt.legend()

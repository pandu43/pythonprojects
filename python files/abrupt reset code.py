...
for i in range(num_outputs):
    ...
    NoOfHotspotsBR = NoOfHotspots
    NoOfColdspotsBR = NoOfColdspots
   ...
   # plt.legend()
    ...
    exp_file = "/home/lavanya/Documents/abrupt reset exp data.ods"
    ...
    plt.semilogy(x1, y1, '-', color='red', label='exp data')
    plt.semilogy(x2, y2, '-', color='red', label='exp data')

        #plt.figure(2)
       # plt.semilogy(RampRate * np.array(TimeSeries_FWRESET), formationrate_Reset, '-', color='blue', label='creation')
       # plt.semilogy(RampRate * np.array(TimeSeries_FWRESET), annhilationrate_Reset, '--', color='red',
            #         label='annhilation')

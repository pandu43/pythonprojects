        ...
        with Pool(processes=num_workers) as pool:
            for batch_idx in range(num_batches):
                ...
                if start >= total_challenges:
                    break
                print(f"Processing batch {batch_idx+1}/{num_batches} ({start}–{end-1})")
...
def main_HRS(challenge_size, crp, save_csv=True, Binary=True, runs=10, timestamp=""):
    ...
    input_file = os.path.join('RRAM_1M_data', '1.9V-1us 1million data.csv')
    puf = PUFDesign(crp, input_file, usecols=[1], skiprows=[0])
    file_name = os.path.join('Result', 'TRAN_result', "TRNG_bitstream_data")
...
if __name__ == "__main__":
    try:
        ...
        main_HRS(challenge_size, crp, save_csv=True, Binary=True, runs=1)
    except EOFError:

...
for idx, pamp in enumerate(pulse_amplitudes):
    ...
    for i in range(num_outputs):
        ...
        time_array = np.array(TimeSeries_FWRESET)  # Use what you save each cycle
        current_array = np.abs(np.array(CurrentSeries_FWRESET))
        resistance = pamp / current_array
        ax.plot(
            [pamp] * len(time_array),
            time_array,
            resistance,
...
ax.set_xticks(pulse_amplitudes)
ax.set_xticklabels([f"{v:.2f}V" for v in pulse_amplitudes])
ax.view_init(elev=30, azim=132)  # Try 120, 130, 140 for azim
plt.tight_layout()
plt.show()

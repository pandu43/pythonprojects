...
pulse_amplitudes = [1.23,1.27,1.3,1.33,1.4]
pulse_duration = 1e-3
# Choose distinct colors for each amplitude
# Normalize amplitudes for color mapping
norm = plt.Normalize(min(pulse_amplitudes), max(pulse_amplitudes))
cmap = plt.get_cmap('rainbow')
plt.figure(figsize=(8,6))
# === Loop over amplitudes ===
for idx, pamp in enumerate(pulse_amplitudes):
    color = cmap(norm(pamp))
    ...
    for i in range(num_outputs):
        ...
        resistance = pamp / np.array(CurrentSeries_FWRESET)
        # ✅ MASK OUT values > 2e9
        #mask = resistance <= 1e10
        ...
        # ✅ Add ONE legend entry for this amplitude
        # ✅ Legend *only voltage*, no “Vp =”
    plt.plot([], [], '-',color=color, linewidth=1,
                label=f"{pamp}")
    # === legend ===
    legend = plt.legend(
      #  title="Vp (V) =",
        title=r"$\mathbf{V_p\ (V) = }$",
        fontsize=14,
        title_fontsize=16,
        frameon=False )
    # ✅ Make the title bold
    ...
    # ✅ Make legend entries bold
    for text in legend.get_texts():
        text.set_fontweight('bold')
        text.set_color('black')
    # ✅ Make handles visible and clean
    for handle in legend.legendHandles:
        handle.set_alpha(1.0)
        handle.set_linewidth(2)

plt.xlim(1e-9, 1.2e-3)
plt.ylim(1.5e3,2.5e9)

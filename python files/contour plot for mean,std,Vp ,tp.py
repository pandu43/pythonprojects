...
#pulse_amplitudes = np.arange(0.9,2.5,0.01)  # Step size 0.1V
#pulse_durations = np.array([5E-9,5E-8,5E-7,5E-6,5E-5,5e-4,5E-3])
# Choose 10 amplitudes between 0.9 and 2.4 V
pulse_amplitudes = np.linspace(0.9, 2.4, 23)
# Choose 10 durations between 1e-9 and 1e-3 (log scale)
pulse_durations = np.logspace(-9, -3, 23)
...
median_values = np.median(all_cycles_data, axis=2)
...
contour = plt.contourf(X, Y, median_values, cmap='rainbow', norm=LogNorm())
cbar = plt.colorbar(contour)
cbar.set_label('Median  (log scale)')
plt.yscale('log')
plt.xlabel('Pulse Amplitude (V)')
plt.ylabel('Pulse Duration (s)')
plt.title('Median vs Pulse Amplitude and Duration')
plt.show()
...
df_std = pd.DataFrame(std_values, index=[f"Amplitude_{amp}" for amp in pulse_amplitudes],
                       columns=[f"Duration_{dur}" for dur in pulse_durations])
df_median = pd.DataFrame(median_values, index=[f"Amplitude_{amp}" for amp in pulse_amplitudes],
                          columns=[f"Duration_{dur}" for dur in pulse_durations])

# Define file paths for three separate Excel files
mean_filepath = os.path.join(output_folder, 'mean_hotspots.xlsx')
std_filepath = os.path.join(output_folder, 'std_hotspots.xlsx')
median_filepath = os.path.join(output_folder, 'median_hotspots.xlsx')

# Save each DataFrame as a separate Excel file
df_mean.to_excel(mean_filepath, sheet_name='Mean Hotspots')
df_std.to_excel(std_filepath, sheet_name='Std Dev Hotspots')
df_median.to_excel(median_filepath, sheet_name='Median Hotspots')

print(f"🎯 All files saved in the '{output_folder}' folder:")
print(f"- {mean_filepath}")
print(f"- {std_filepath}")
print(f"- {median_filepath}")

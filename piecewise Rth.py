...
def find_Vreset_from_simulation(AppliedBias_list, NoOfHotspots_list):
    for i in range(1, len(NoOfHotspots_list)):
        if NoOfHotspots_list[i] < NoOfHotspots_list[i - 1]:  # Annihilation event
            return AppliedBias_list[i]
    return None  # No annihilation detected

def calculate_piecewise_Rth(AppliedBias, phase, Vreset=None):
    if phase == 'SET':
        return 5e7
    elif phase == 'RESET':
        if Vreset is None:
            return 3e6  # default value before annihilation starts
        elif AppliedBias < Vreset:
            return 3e6
        else:
            #print ("Vreset",Vreset)
            return  3e6 + 7e6 * (AppliedBias - Vreset)
    else:
        raise ValueError("Phase must be 'SET' or 'RESET'")
...
def KmcProcess_FWSet(RampRate, Thickness, NoOfColdspots , NoOfHotspots,
    ...
    while (CurrentCalculation(AppliedBias, NoOfHotspots) <= ComplianceCurrent):
        ...
        Rth = calculate_piecewise_Rth(AppliedBias, 'SET', Vreset=None)
...
def KmcProcess_FWReset(RampRate, Thickness, NoOfColdspots , NoOfHotspots,
    ...
    while (AppliedBias <= 2):
       ...
       # if StepIndex > 1e0:
        ...
        Vreset = find_Vreset_from_simulation(AppliedBiasSeries_FWRESET, NoOfHotspotsSeries_FWRESET)

        Rth = calculate_piecewise_Rth(AppliedBias, 'RESET', Vreset)
...
for i in range(num_outputs):
   ...
   # plt.legend()
    ...
    plt.figure(2)
    plt.plot(Voltage_series_Reset, Rth_series_reset, '-', color='blue', label='Rth')
    plt.xlabel('Voltage (V)')
    plt.ylabel(' Rth  ')

    plt.figure(3)
   # plt.plot(RampRate * np.array(TimeSeries_FWRESET), np.log10(NoOfColdspotsSeries_FWRESET), 'o', color='red', label='coldspots')
    plt.plot(RampRate * np.array(TimeSeries_FWRESET), (np.log10(1.05E6) - np.log10(NoOfHotspotsSeries_FWRESET)), 'o', color= 'red')
    plt.xlabel('Voltage (V)')
    plt.ylabel(' log(InitialNoOfcoldspots - NoOfHotspots) ')
    plt.legend()
